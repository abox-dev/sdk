...
const API_KEY_A = `ab_${'a'.repeat(40)}`
const API_KEY_B = `ab_${'b'.repeat(40)}`
...
const envOverrides = {
  AGENTBOX_DOMAIN: DOMAIN_ENV,
  AGENTBOX_API_KEY: TEST_API_KEY,
  AGENTBOX_API_URL: undefined,
  AGENTBOX_SANDBOX_URL: undefined,
  AGENTBOX_DEBUG: undefined,
}
...
test('client.Sandbox.create uses the client config instead of env vars', async () => {
  const client = new AgentBox({ apiKey: API_KEY_A, domain: DOMAIN_A })
...
test('two clients with different configs stay isolated', async () => {
  const clientA = new AgentBox({ apiKey: API_KEY_A, domain: DOMAIN_A })
  const clientB = new AgentBox({ apiKey: API_KEY_B, domain: DOMAIN_B })
...
test('mutating the options object does not change the bound config', async () => {
  ...
  const client = new AgentBox(opts)
