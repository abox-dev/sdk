...
test('transform callback sees every registered iam token', async () => {
  ...
  expect(
    ...
    'X-Json': JSON.stringify({
      aws: '${agentbox.identity.tokens.aws}',
      gcp: '${agentbox.identity.tokens.gcp}',
    }),
  })
})
...
test.for([
  ['a}b', 'closing brace'],
  ['a{b', 'opening brace'],
  ['aws}${agentbox.identity.tokens.gcp', 'smuggled placeholder'],
  ['a\nb', 'control character'],
  ['', 'empty'],
])(
...
test('an ordinary iam token name is accepted', async () => {
  ...
  expect(
    ...
    Authorization: 'Bearer ${agentbox.identity.tokens.aws.prod-1_x}',
  })
})
