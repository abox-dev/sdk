...
test('does not pass custom connection headers to envd RPC requests', async () => {
  ...
  await mocks.transportFetch('https://sandbox.agentbox.dev/rpc', {
  ...
  assert.equal(headers.get('User-Agent')?.startsWith('agentbox-js-sdk/'), true)
  assert.equal(headers.get('Agentbox-Sandbox-Id'), 'sbx-test')
