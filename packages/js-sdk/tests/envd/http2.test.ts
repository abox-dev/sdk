...
afterEach(() => {
  ...
  delete process.env.AGENTBOX_ENVD_RPC_CONNECTIONS
  delete process.env.AGENTBOX_ENVD_INFLIGHT_REQUESTS
  delete process.env.AGENTBOX_ENVD_RPC_INFLIGHT_REQUESTS
})
...
test('reads RPC stream dispatcher connection limit from env', async () => {
  process.env.AGENTBOX_ENVD_RPC_CONNECTIONS = '200'
...
test('getEnvdRpcConnectionLimit throws on malformed env value', async () => {
  process.env.AGENTBOX_ENVD_RPC_CONNECTIONS = 'bogus'

  const { getEnvdRpcConnectionLimit } = await import('../../src/envd/http2')

  expect(() => getEnvdRpcConnectionLimit()).toThrow(
    /AGENTBOX_ENVD_RPC_CONNECTIONS/
  )
})

test('getEnvdInflightLimit throws on malformed env value', async () => {
  process.env.AGENTBOX_ENVD_INFLIGHT_REQUESTS = 'bogus'

  const { getEnvdInflightLimit } = await import('../../src/envd/http2')

  expect(() => getEnvdInflightLimit()).toThrow(
    /AGENTBOX_ENVD_INFLIGHT_REQUESTS/
  )
})

test('getEnvdRpcInflightLimit throws on malformed env value', async () => {
  process.env.AGENTBOX_ENVD_RPC_INFLIGHT_REQUESTS = 'bogus'

  const { getEnvdRpcInflightLimit } = await import('../../src/envd/http2')

  expect(() => getEnvdRpcInflightLimit()).toThrow(
    /AGENTBOX_ENVD_RPC_INFLIGHT_REQUESTS/
  )
})

test('inflight limit env vars return 0 when explicitly disabled', async () => {
  process.env.AGENTBOX_ENVD_INFLIGHT_REQUESTS = '0'
  process.env.AGENTBOX_ENVD_RPC_INFLIGHT_REQUESTS = '0'
...
test('getEnvdInflightLimit throws on negative env value', async () => {
  process.env.AGENTBOX_ENVD_INFLIGHT_REQUESTS = '-1'

  const { getEnvdInflightLimit } = await import('../../src/envd/http2')

  expect(() => getEnvdInflightLimit()).toThrow(
    /AGENTBOX_ENVD_INFLIGHT_REQUESTS=-1/
  )
})

test('getEnvdRpcInflightLimit throws on negative env value', async () => {
  process.env.AGENTBOX_ENVD_RPC_INFLIGHT_REQUESTS = '-5'

  const { getEnvdRpcInflightLimit } = await import('../../src/envd/http2')

  expect(() => getEnvdRpcInflightLimit()).toThrow(
    /AGENTBOX_ENVD_RPC_INFLIGHT_REQUESTS=-5/
  )
})
