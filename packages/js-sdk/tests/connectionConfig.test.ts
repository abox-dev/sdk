...
beforeEach(() => {
  originalEnv = {
    AGENTBOX_API_URL: process.env.AGENTBOX_API_URL,
    AGENTBOX_DOMAIN: process.env.AGENTBOX_DOMAIN,
    AGENTBOX_SANDBOX_URL: process.env.AGENTBOX_SANDBOX_URL,
    AGENTBOX_DEBUG: process.env.AGENTBOX_DEBUG,
  }
})
...
test('api_url defaults correctly', () => {
  ...
  delete process.env.AGENTBOX_API_URL
  delete process.env.AGENTBOX_DOMAIN
  delete process.env.AGENTBOX_DEBUG

  const config = new ConnectionConfig()
  assert.equal(config.apiUrl, 'https://api.agentbox-runtime.ru')
})
...
test('api_url in env var', () => {
  process.env.AGENTBOX_API_URL = 'http://localhost:8080'
...
test('api_url has correct priority', () => {
  process.env.AGENTBOX_API_URL = 'http://localhost:1111'
...
test('sandbox_url defaults to stable sandbox host in production', () => {
  delete process.env.AGENTBOX_SANDBOX_URL
  delete process.env.AGENTBOX_DOMAIN
  delete process.env.AGENTBOX_DEBUG
...
test('sandbox_direct_url keeps per-sandbox host in production', () => {
  ...
  assert.equal(
    ...
    'https://49983-sbx-test.agentbox.app'
  )
})
...
test('sandbox_url keeps per-sandbox host outside production', () => {
  ...
  const config = new ConnectionConfig({ domain: 'agentbox.dev' })

  assert.equal(
    config.getSandboxUrl('sbx-test', {
      sandboxDomain: 'sandbox.agentbox.dev',
      envdPort: 49983,
    }),
    'https://49983-sbx-test.sandbox.agentbox.dev'
  )
})

test('sandbox_url in args has priority', () => {
  process.env.AGENTBOX_SANDBOX_URL = 'https://sandbox.from-env'
...
test('debug defaults to AGENTBOX_DEBUG env var', () => {
  process.env.AGENTBOX_DEBUG = 'true'
...
test('setIntegration appends the integration to the user agent', () => {
  ...
  assert.equal(
    config.headers?.['User-Agent']?.startsWith('agentbox-js-sdk/'),
    true
  )
  assert.equal(
...
test('clearing the integration propagates to config rebuilds', () => {
  ...
  assert.equal(
    rebuiltConfig.headers?.['User-Agent']?.startsWith('agentbox-js-sdk/'),
    true
  )
  assert.equal(
