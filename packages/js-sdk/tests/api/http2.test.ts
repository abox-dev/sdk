...
afterEach(() => {
  ...
  delete process.env.AGENTBOX_API_CONNECTIONS
  delete process.env.AGENTBOX_API_INFLIGHT_REQUESTS
})
...
test('getApiConnectionLimit throws on a malformed env value', async () => {
  process.env.AGENTBOX_API_CONNECTIONS = 'not-a-number'

  const { getApiConnectionLimit } = await import('../../src/api/http2')

  expect(() => getApiConnectionLimit()).toThrow(/AGENTBOX_API_CONNECTIONS/)
})

test('getApiInflightLimit throws on a malformed env value', async () => {
  process.env.AGENTBOX_API_INFLIGHT_REQUESTS = 'not-a-number'

  const { getApiInflightLimit } = await import('../../src/api/http2')

  expect(() => getApiInflightLimit()).toThrow(/AGENTBOX_API_INFLIGHT_REQUESTS/)
})

test('getApiInflightLimit returns 0 when explicitly disabled', async () => {
  process.env.AGENTBOX_API_INFLIGHT_REQUESTS = '0'
...
test('getApiInflightLimit throws on negative env value', async () => {
  process.env.AGENTBOX_API_INFLIGHT_REQUESTS = '-5'

  const { getApiInflightLimit } = await import('../../src/api/http2')

  expect(() => getApiInflightLimit()).toThrow(
    /AGENTBOX_API_INFLIGHT_REQUESTS=-5/
  )
})
