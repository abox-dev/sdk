...
export interface ConnectionOpts {
   ...
   *
   * @default AGENTBOX_API_KEY // environment variable
   */
  apiKey?: string
  /**
   * Domain to use for the API.
   *
   * @default AGENTBOX_DOMAIN // environment variable or `agentbox-runtime.ru`
   */
  domain?: string
  /**
   * API Url to use for the API.
   * @internal
   * @default AGENTBOX_API_URL // environment variable or `https://api.${domain}`
   */
  apiUrl?: string
  /**
   * Sandbox Url to use for the API.
   * @internal
   * @default AGENTBOX_SANDBOX_URL // environment variable, `https://sandbox.${domain}`
   */
  sandboxUrl?: string
  /**
   * If true the SDK starts in the debug mode and connects to the local envd API server.
   * @internal
   * @default AGENTBOX_DEBUG // environment variable or `false`
   */
  debug?: boolean
  /**
...
function abortWithReason(controller: AbortController, reason: unknown) {
  ...
  ;(controller as { __agentboxAbortReason?: unknown }).__agentboxAbortReason =
    reason
...
export class ConnectionConfig {
  ...
  private static readonly sdkUserAgentPrefix = 'agentbox-js-sdk/'
  ...
  /**
   * Identify traffic from an integration wrapping the AgentBox SDK by appending
   * `integration` (e.g. `'agentbox-code-interpreter/0.1.0'`) to the `User-Agent`
  ...
  constructor(opts?: ConnectionOpts) {
    ...
    const inheritedHeaders = (
      opts as
        | (ConnectionOpts & { headers?: Record<string, string> })
        | undefined
    )?.headers
    this.headers = { ...(opts?.apiHeaders ?? inheritedHeaders ?? {}) }
  ...
  /**
   * Merge connection options bound to a class (e.g. by an `AgentBox` client) with
  ...
  private static get domain() {
    return getEnvVar('AGENTBOX_DOMAIN') || 'agentbox-runtime.ru'
  }

  private static get apiUrl() {
    return getEnvVar('AGENTBOX_API_URL')
  }

  private static get sandboxUrl() {
    return getEnvVar('AGENTBOX_SANDBOX_URL')
  }

  private static get debug() {
    return (getEnvVar('AGENTBOX_DEBUG') || 'false').toLowerCase() === 'true'
  }

  private static get apiKey() {
    return getEnvVar('AGENTBOX_API_KEY')
  }
  ...
  getSandboxUrl(
    ...
    // The stable sandbox host is only guaranteed for AgentBox prod; the various other hosted domains may not serve sandbox.<domain> yet and will follow up once those are updated.
...
/**
 * Base class for the resource classes (`Sandbox` and `Template`) whose static methods build a `ConnectionConfig` from per-call
 * options. An {@link AgentBox} client exposes subclasses of these with its own
...
export class ClientFactory {
  /**
   * Connection options bound to this class by an {@link AgentBox} client.
   *
