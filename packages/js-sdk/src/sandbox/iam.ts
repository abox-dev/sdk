 ...
 * `'${agentbox.identity.tokens.'` and the next `}`, then looks that name up in the
...
export function validateIamTokenName(name: string): void {
  if (name.length === 0 || INVALID_IAM_TOKEN_NAME_CHARS.test(name)) {
    throw new InvalidArgumentError(
      `iam token name ${JSON.stringify(name)} is not usable: a token name cannot be empty or contain '{', '}' or control characters, because it is interpolated into the '\${agentbox.identity.tokens.<name>}' placeholder the egress proxy resolves.`
    )
  }
}
...
export function iamTokenPlaceholder(name: string): string {
  ...
  return `\${agentbox.identity.tokens.${name}}`
}
...
export function iamTokenPlaceholders(
  ...
  const resolveUnregistered = (prop: string): string => {
    ...
    const hint =
      tokenNames.length === 0
        ? `Pass it to Sandbox.create as iam: { tokens: { '${prop}': { audience, tokenType } } }.`
