...
{
  private defaultBaseImage: string =
    'docker.io/retailcrm/agentbox-base:v0.0.1@sha256:493b753044eb82f90b99afb3974033591ccbd7037b39e7963dfe49f38790376e'
   ...
   * @throws Error if the template is based on another AgentBox template
   */
  ...
  static async build(
    ...
    const buildOpts = this.resolveOpts(options) ?? {}
   ...
   * Templates based on other AgentBox templates cannot be converted because they
   * may use features not available in standard Dockerfiles.
   *
   * @returns Dockerfile string representation
   * @throws Error if template is based on another AgentBox template or has no base image
   */
   ...
   *
   * @param client API client for communicating with AgentBox backend
   * @param name Template name in 'name' or 'name:tag' format
