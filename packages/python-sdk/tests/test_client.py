...
def api_server(monkeypatch):
    ...
    monkeypatch.setenv("AGENTBOX_API_KEY", ENV_API_KEY)
    monkeypatch.setenv("AGENTBOX_DOMAIN", ENV_DOMAIN)
...
def test_client_sandbox_uses_client_config(api_server):
    client = AgentBox(api_key=API_KEY_A, domain=DOMAIN_A, api_url=api_server)
...
def test_two_clients_stay_isolated(api_server):
    client_a = AgentBox(api_key=API_KEY_A, domain=DOMAIN_A, api_url=api_server)
    client_b = AgentBox(api_key=API_KEY_B, domain=DOMAIN_B, api_url=api_server)
