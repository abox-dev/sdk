...
from agentbox.template import utils as template_utils
from agentbox.exceptions import FileUploadException
from agentbox.template.consts import FILE_UPLOAD_TIMEOUT_SECONDS
from agentbox.template_async.build_api import upload_file


# Regression test for agentbox-dev/agentbox#1243 — upload_file must set Content-Length
...
async def _capture_upload_timeout(tmp_path, request_timeout=None):
    ...
    try:
        ...
        with mock.patch(
            "agentbox.template_async.build_api.httpx.AsyncClient",
            side_effect=spy_client,
        ):
            await upload_file(
                api_client=client,
                file_name="*.txt",
...
async def test_upload_file_ignores_post_upload_close_failure(tmp_path):
    ...
    try:
        client = AuthenticatedClient(base_url="http://test", token="test")
        with mock.patch(
            "agentbox.template_async.build_api.tar_file_stream",
