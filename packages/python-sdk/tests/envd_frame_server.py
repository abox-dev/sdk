...
``agentbox.envd.client_sync``/``client_async`` wiring over a plaintext
...
from agentbox.envd.api import ENVD_API_HEALTH_ROUTE
from agentbox.envd.client_shared import ENVD_JSON_CODEC, ENVD_RPC_COMPRESSION
from agentbox.envd.interceptors import build_interceptors
from agentbox.envd.process.process_connect import ProcessClient, ProcessClientSync
from agentbox.envd.process.process_pb import ConnectResponse, ProcessEvent
...
def make_config(logger: Optional[logging.Logger] = None) -> ConnectionConfig:
    ...
    return ConnectionConfig(api_key="ab_" + "0" * 40, logger=logger)


# The factories in agentbox.envd.client_sync/client_async use the shared TLS
