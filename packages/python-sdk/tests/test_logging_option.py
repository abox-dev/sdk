...
from agentbox import AsyncSandbox, ConnectionConfig, Sandbox
from agentbox.envd.interceptors import LoggingInterceptor, build_interceptors
...
def test_connection_config_stores_logger():
    ...
    config = ConnectionConfig(api_key="ab_" + "0" * 40, logger=custom)
    assert config.logger is custom


def test_connection_config_logger_defaults_to_none():
    config = ConnectionConfig(api_key="ab_" + "0" * 40)
...
def test_get_api_params_propagates_stored_logger():
    ...
    no_logger = ConnectionConfig(api_key="ab_" + "0" * 40)
