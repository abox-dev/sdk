...
from agentbox import SandboxIamToken, SandboxNetworkOpts
        ...
        "api.internal.example.com": [
            {
                "transform": {
                    "headers": {
                        "Authorization": "Bearer ${agentbox.identity.tokens.aws}",
                    },
                },
            },
        ],
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param("aws}${agentbox.identity.tokens.gcp", id="smuggled-placeholder"),
        pytest.param("a\nb", id="control-char"),
...
def test_ordinary_iam_token_names_are_accepted():
    ...
    assert body["rules"].to_dict() == {
        "api.internal.example.com": [
            {
                "transform": {
                    "headers": {"A": "${agentbox.identity.tokens.aws.prod-1_x}"}
                }
            },
        ],
    }
            ...
            "api.internal.example.com": [
                {
                    "transform": {
                        "headers": {
                            "Authorization": "Bearer ${agentbox.identity.tokens.aws}",
                            "X-Also": "${agentbox.identity.tokens.gcp}",
                        },
                    },
                },
            ],
