...
def test_api_url_defaults_correctly(monkeypatch):
    ...
    monkeypatch.delenv("AGENTBOX_API_URL", raising=False)

    config = ConnectionConfig()
    assert config.api_url == "https://api.agentbox-runtime.ru"
...
def test_sandbox_url_uses_stable_host_for_supported_domain():
    config = ConnectionConfig(domain="agentbox-runtime.ru")
...
def test_sandbox_url_uses_explicit_url_first():
    ...
    assert (
        config.get_sandbox_url("sandbox-id", "agentbox.app")
...
def test_sandbox_url_falls_back_to_per_sandbox_host_for_unsupported_subdomain():
    config = ConnectionConfig(domain="agentbox.dev")

    assert (
        config.get_sandbox_url("sandbox-id", "sandbox.agentbox.dev")
        == "https://49983-sandbox-id.sandbox.agentbox.dev"
    )
...
def test_sandbox_url_debug_uses_localhost():
    ...
    assert (
        config.get_sandbox_url("sandbox-id", "agentbox.app") == "http://localhost:49983"
    )
...
def test_get_host_keeps_per_sandbox_host_for_supported_domain():
    ...
    assert (
        config.get_host("sandbox-id", "agentbox.app", 8888)
        == "8888-sandbox-id.agentbox.app"
    )
...
def test_sandbox_direct_url_keeps_per_sandbox_host_for_supported_domain():
    ...
    assert (
        config.get_sandbox_direct_url("sandbox-id", "agentbox.app")
        == "https://49983-sandbox-id.agentbox.app"
    )
...
def test_debug_false_overrides_env_var(monkeypatch):
    monkeypatch.setenv("AGENTBOX_DEBUG", "true")
...
def test_set_integration_appends_to_user_agent():
    ...
    try:
        ...
        assert config.headers["User-Agent"].startswith("agentbox-python-sdk/")
...
def test_integration_survives_api_param_rebuilds(monkeypatch):
    ...
    assert rebuilt_config.get_api_params(api_headers={"X-Test": "1"})["api_headers"][
        "User-Agent"
...
def test_cleared_integration_does_not_leak_into_api_param_rebuilds():
    ...
    assert not params["api_headers"]["User-Agent"].endswith(" testing/version")
...
def test_per_call_user_agent_override_wins_in_api_params(monkeypatch):
    ...
    assert params["api_headers"]["User-Agent"] == "custom/1.0"
...
def test_per_call_api_headers_user_agent_wins_over_headers():
    ...
    assert params["api_headers"]["User-Agent"] == "from-api-headers/1.0"
