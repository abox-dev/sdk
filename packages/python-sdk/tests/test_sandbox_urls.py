...
def create_sandbox(envd_access_token=None):
    return SandboxBase(
        ...
        sandbox_domain="agentbox-runtime.ru",
        envd_version=Version("0.4.0"),
        envd_access_token=envd_access_token,
        traffic_access_token=None,
        connection_config=ConnectionConfig(domain="agentbox-runtime.ru"),
    )
...
def test_file_urls_use_direct_sandbox_host_when_envd_api_uses_stable_host():
    ...
    assert sandbox.envd_api_url == "https://sandbox.agentbox-runtime.ru"
    assert sandbox.envd_direct_url == "https://49983-sandbox-id.agentbox-runtime.ru"
    assert (
        sandbox.download_url("/tmp/a.txt")
        == "https://49983-sandbox-id.agentbox-runtime.ru/files?path=%2Ftmp%2Fa.txt"
    )
    assert (
