...
from agentbox.api import ProxyConfig, proxy_to_config
from agentbox.connection_config import ConnectionConfig, ProxyTypes
from agentbox.envd import client_async, client_sync
from agentbox.envd.process.process_connect import ProcessClient, ProcessClientSync
...
def test_rpc_clients_run_on_the_shared_pool(test_api_key, monkeypatch):
    ...
    client_sync.create_rpc_client(
        ProcessClientSync, "https://sandbox.agentbox.app", config
    )
    client_async.create_rpc_client(
        ProcessClient, "https://sandbox.agentbox.app", config
    )
...
def test_shared_pool_retries_connects():
    # `AGENTBOX_CONNECTION_RETRIES` must flow into the retry layer of the shared
    # pool, which every stack now inherits.
    from agentbox.api import connection_retries
