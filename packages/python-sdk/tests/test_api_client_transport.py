...
import agentbox.api.client_async as api_client_async
import agentbox.api.client_sync as api_client_sync
from agentbox.api import pool_idle_timeout, pool_max_idle_per_host, proxy_to_config
from agentbox.api.client_async import get_api_client as get_async_api_client
from agentbox.api.client_async import get_envd_api as get_async_envd_api
from agentbox.api.client_async import (
    get_pyqwest_transport as get_async_pyqwest_transport,
)
from agentbox.api.client_async import get_transport as get_async_transport
from agentbox.api.client_sync import get_api_client as get_sync_api_client
from agentbox.api.client_sync import get_envd_api as get_sync_envd_api
from agentbox.api.client_sync import get_pyqwest_transport as get_sync_pyqwest_transport
from agentbox.api.client_sync import get_transport as get_sync_transport
from agentbox.connection_config import READ_TIMEOUT, ConnectionConfig
...
def test_sync_transports_keyed_by_http_version(test_api_key):
    ...
    try:
        ...
        envd_negotiated = get_sync_transport(config)
        envd_http1 = get_sync_transport(config, http2=False)
        ...
        # version — `get_transport` is `get_transport` under another name.
        ...
        assert get_sync_transport(config, http2=False) is envd_http1
        assert (
...
def test_sync_transports_pass_http_version_to_pyqwest(test_api_key, monkeypatch):
    # `http_version=None` leaves the version to ALPN (HTTP/2 against the AgentBox
    ...
    try:
        ...
        # (`get_transport(config, http2=False)` would be a cache hit and
...
def test_sync_envd_api_client_wiring(test_api_key):
    ...
    client = get_sync_envd_api(config, "https://sandbox.agentbox.app")
    streaming = get_sync_envd_api(
        config, "https://sandbox.agentbox.app", for_streaming=True
    )

    try:
        assert client.base_url == "https://sandbox.agentbox.app"
...
async def test_async_transports_keyed_by_http_version(test_api_key):
    ...
    try:
        ...
        envd_negotiated = get_async_transport(config)
        envd_http1 = get_async_transport(config, http2=False)
        ...
        assert get_async_transport(config, http2=False) is envd_http1
        assert (
...
async def test_async_envd_api_client_wiring(test_api_key):
    ...
    client = get_async_envd_api(config, "https://sandbox.agentbox.app")
