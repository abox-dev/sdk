...
from agentbox.template_sync.build_api import upload_file
...
def _capture_upload_timeout(tmp_path, request_timeout=None):
    ...
    try:
        ...
        with mock.patch(
            "agentbox.template_sync.build_api.httpx.Client", side_effect=spy_client
        ):
            upload_file(
                api_client=client,
                file_name="*.txt",
...
def test_upload_file_ignores_post_upload_close_failure(tmp_path):
    ...
    try:
        client = AuthenticatedClient(base_url="http://test", token="test")
        with mock.patch(
            "agentbox.template_sync.build_api.tar_file_stream",
