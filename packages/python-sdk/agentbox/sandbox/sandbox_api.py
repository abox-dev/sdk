...
class _SandboxNetworkTransformIam:
    ...
    string ``"${agentbox.identity.tokens.aws}"``, which the egress proxy replaces
...
class SandboxNetworkOpts(TypedDict):
    ...
    Supports ${PORT} variable. Defaults to "${PORT}-sandboxid.agentbox-runtime.ru".
...
class SandboxIamOpts(TypedDict, total=False):
    ...
    Each value contains ``audience`` and ``token_type``.
...
def _build_network_egress(
    ...
    ``rules``) used by both the create and update
