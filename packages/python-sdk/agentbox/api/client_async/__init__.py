...
# (`agentbox.envd.client_async`) and the volume content API
# (`agentbox.volume.client_async`). reqwest pools per host internally, so a single
        ...
        if transport is None:
            transport = ConnectionRetryTransport(
                HTTPTransport(
                    ...
                    # (HTTP/2 against the AgentBox API and envd) and uses HTTP/1 for
...
def get_transport(
    ...
    version (HTTP/2 against the AgentBox API), like the http2-enabled httpx
