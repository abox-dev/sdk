...
from agentbox.sandbox_domains import is_supported_sandbox_domain
...
proxy type as the transports move off httpx. :func:`agentbox.api.proxy_to_config`
...
def merge_api_params(
    ...
    """
    Merge the API params bound to a class (e.g. by an :class:`agentbox.AgentBox` client)
...
class ClientFactory:
    """
    Base class for the resource classes (`Sandbox` and `Template`) whose classmethods build a :class:`ConnectionConfig` from per-call
    params. An :class:`agentbox.AgentBox` client exposes subclasses of these with its own
    ...
    """API params bound to this class by an :class:`agentbox.AgentBox` client.
...
class ConnectionConfig:
    ...
    def set_integration(cls, integration: Optional[str]) -> None:
        ...
        ``integration`` (e.g. ``"agentbox-code-interpreter/0.1.0"``) to the
    ...
    @staticmethod
    def _domain():
        return os.getenv("AGENTBOX_DOMAIN") or "agentbox-runtime.ru"
    ...
    def _build_user_agent() -> str:
        user_agent_parts = [f"agentbox-python-sdk/{package_version}"]
    ...
    def get_api_params(
        ...
        **opts: Unpack[ApiParams],
        ...
        if self._user_agent_is_sdk_built:
            per_call_user_agent = (api_headers or {}).get("User-Agent")
    ...
    def sandbox_headers(self):
        """
        We need this separate as we use the same header for AgentBox access token to API and envd access token to sandbox.
        """
        return {
