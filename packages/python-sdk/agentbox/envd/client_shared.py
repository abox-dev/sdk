...
clients in `agentbox.api` use (`get_pyqwest_transport`), so RPCs and the envd HTTP
...
:mod:`agentbox.envd.client_sync` and :mod:`agentbox.envd.client_async`, mirroring the
`agentbox.api.client_sync`/`client_async` layout. This module exists because
`agentbox/envd/__init__.py`, the natural home by that analogy, is owned by the
