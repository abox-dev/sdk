...
const templateId =
  process.env.AGENTBOX_CLI_BACKEND_TEMPLATE_ID ||
  process.env.AGENTBOX_TEMPLATE_ID ||
  'base'
const sandboxTimeoutMs = parseEnvInt(
  'AGENTBOX_CLI_BACKEND_SANDBOX_TIMEOUT_MS',
  20_000
)
const perTestTimeoutMs = parseEnvInt(
  'AGENTBOX_CLI_BACKEND_TEST_TIMEOUT_MS',
  30_000
)
...
describe('sandbox cli backend integration', () => {
  ...
  integrationTest(
    ...
    async () => {
      const listResult = runCliInSandbox([
        'sandbox',
        'list',
        '--format',
        'json',
      ...
      expect(sandboxExistsInList(listResult.stdout, sandbox.sandboxId)).toBe(
        true
      )
    }
  )
